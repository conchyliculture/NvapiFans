...
static const unsigned short normal_i2c[] = {ASUS_FC2_DEVICE_ADDR,
                                            I2C_CLIENT_END};

static const struct i2c_device_id asus_fc2_id[] = {{"asus_fc2", 0}, {}};
...
static int asus_fc2_read(struct device *dev, enum hwmon_sensor_types type,
...
static int asus_fc2_write(struct device *dev, enum hwmon_sensor_types type,
                          u32 attr, int channel, long val) {
  struct regmap *regmap = dev_get_drvdata(dev);
  switch (attr) {
  case hwmon_pwm_input:
...
static int asus_fc2_probe(struct i2c_client *client,
  ...
  hwmon_dev = devm_hwmon_device_register_with_info(dev, client->name, regmap,
                                                   &asus_fc2_chip_info, NULL);
  return PTR_ERR_OR_ZERO(hwmon_dev);
}

/* Return 0 if detection is successful, -ENODEV otherwise */
static int asus_fc2_detect(struct i2c_client *new_client,
  ...
  if (regval_low != ASUS_FC2_IT8915_LOW_ID) {
    //        printk(KERN_INFO "bad2 %d!=%d", regval_low,
    //        ASUS_FC2_IT8915_LOW_ID);
    return -ENODEV;
  }
  if (regval_high != ASUS_FC2_IT8915_HIGH_ID) {
    //        printk(KERN_INFO "bad3 %d!=%d", regval_high,
    //        ASUS_FC2_IT8915_HIGH_ID);
    return -ENODEV;
  }
